...
exports.get_all_vin = function (req, res) {
    var localsql = 
        "select DISTINCT VIN from " + tb_LOC_BATCH +" ;"
...
// 可能一辆车会和另一辆车的maxtime冲突，但其实他自己还有自己的maxtime，导致重复
exports.get_all_location_batch = function (req, res) {
    var localsql = 
        "select DISTINCT VIN, collectTime, longitude, latitude, angle from " + tb_LOC_BATCH +
...
//         " JOIN (SELECT VIN, MAX(collectTime) as collectTime FROM " + tb_LOC_BATCH + "GROUP BY VIN) t2" +
//         " ON E100_LOC_BATCH.VIN = t2.VIN AND E100_LOC_BATCH.collectTime = t2.collectTime; "
//         //console.log(localsql)
