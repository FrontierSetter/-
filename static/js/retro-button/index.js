...
// ---Retro Button---
//
var buttons = document.querySelectorAll('.rebtn');

for(var i = 0; i < buttons.length; i++) {
  // Click
  buttons[i].addEventListener('mousedown', function() {
    this.classList.add('rebtn-active');
  });
  buttons[i].addEventListener('mouseup', function() {
    this.classList.remove('rebtn-active');
  });

  // Hover
  buttons[i].addEventListener('mouseleave', function() {
    this.classList.remove('rebtn-center', 'rebtn-right', 'rebtn-left', 'rebtn-active');
  });

  buttons[i].addEventListener("mousemove", function(e) {
    var leftOffset = this.getBoundingClientRect().left;
    var rebtnWidth = this.offsetWidth;
    var myPosX = e.pageX;
    var newClass = "";
    // if on left 1/3 width of rebtn
    if(myPosX < (leftOffset + .3 * rebtnWidth) ) {
      newClass = 'rebtn-left'
    } else {
      // if on right 1/3 width of rebtn
      if(myPosX > (leftOffset + .65 * rebtnWidth) ) {
        newClass = 'rebtn-right';
      } else {
        newClass = 'rebtn-center';
      }
    }
    // remove prev class
    var clearedClassList = this.className.replace(/rebtn-center|rebtn-right|rebtn-left/gi, "").trim();
